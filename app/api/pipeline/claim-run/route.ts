...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const queued = await prisma.runRequest.findMany({ where: { status: 'queued' }, orderBy: { created_at: 'asc' } });
    for (const q of queued) {
      ...
      const job = await prisma.runRequest.findUnique({ where: { id: q.id }, include: { project: { select: { title: true } } } });
      if (!job) continue;
      ...
      return NextResponse.json({ job: { id: job.id, projectId: job.project_id, video: job.project.title, repo: job.repo, sha: job.sha, detail: job.detail } });
    }
