...
const TOOLS = [
  ...
  // ── Time tracking ─────────────────────────────────────────────────────────
  {
    ...
    description:
      "THE call for pricing questions. Returns the whole time-tracking picture in one document: every video with its tracked " +
      "minutes and hours, sessions and days worked, and the free-text notes typed on it (what made it slow or fast, what was " +
      "charged) — plus per-client and grand totals. Optionally filter by client. Set include_sessions to also get every " +
      "individual stretch (start, end, minutes) per video.",
    inputSchema: {
      type: "object",
      properties: {
        client: { type: "string", description: "Filter by client name or company (substring, case-insensitive)." },
        include_sessions: { type: "boolean", description: "Also list each video's individual sessions. Default false." },
      },
    },
  },
  {
